...
//	3. 以下兩行代碼是作定位用，若不想做其他位置特效，請保留這兩行代碼。
	...
	$s = "{\\pos($Xoffset[$i],$Yoffset)}{\\1vc($color[0],$color[1],$color[2],$color[3])}{\\2vc(999999,999999,999999,999999)}{\\bord7}{\\blur7}{\\3vc(FFFFFF,FFFFFF,FFFFFF,FFFFFF)}";
	$Upper = "{\\pos($UpperXoffset[$i],$UpperYoffset)}{\\1vc($color[0],$color[1],$color[2],$color[3])}{\\2vc(999999,999999,999999,999999)}{\\bord7}{\\blur7}{\\3vc(FFFFFF,FFFFFF,FFFFFF,FFFFFF)}";
...
//	   須使用{\\K$PreOffset[$i]}{\\K$offset[$i]}。
	$t99 = $start[$i] + 500;
	if($t99>$end[$i]) $t99 = $end[$i];

	$s .= "{\\fad(300,300)}{\\K$PreOffset[$i]}{\\K$offset[$i]}";
	$Upper .= "{\\fad(300,300)}{\\K$PreOffset[$i]}{\\K$offset[$i]}";

//================================================= [ Part D ] ===================================================={\\t($start[$i],$t99,\\bord7\\blur7\\3vc(FFFFFF,FFFFFF,FFFFFF,FFFFFF))}
//事件參數設定及儲存
//
