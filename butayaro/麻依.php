...
//	3. 以下兩行代碼是作定位用，若不想做其他位置特效，請保留這兩行代碼。
	...
	if($color2 != 'FFFFFF'){
		...
		$sClone = "{\\move($Xoffset[$i],$Yoffset,$Xoffset[$i],$Y2,$start[$i], $end[$i])}{\\1vc($color[0],$color[1],$color[2],$color[3])}{\\2vc(FFFFFF,FFFFFF,FFFFFF,FFFFFF)}{\\3vc(000000,000000,000000,000000)}{\\alpha&H66}";
		$UpperClone = "{\\move($UpperXoffset[$i],$UpperYoffset,$UpperXoffset[$i],$YU2,$start[$i], $end[$i])}{\\1vc($color[0],$color[1],$color[2],$color[3])}{\\2vc(FFFFFF,FFFFFF,FFFFFF,FFFFFF)}{\\alpha&H66}{\\3vc(000000,000000,000000,000000)}";
	}
	$s = "{\\pos($Xoffset[$i],$Yoffset)}{\\1vc($color[0],$color[1],$color[2],$color[3])}{\\2vc(FFFFFF,FFFFFF,FFFFFF,FFFFFF)}{\\3vc(000000,000000,000000,000000)}";
	$Upper = "{\\pos($UpperXoffset[$i],$UpperYoffset)}{\\1vc($color[0],$color[1],$color[2],$color[3])}{\\2vc(FFFFFF,FFFFFF,FFFFFF,FFFFFF)}{\\3vc(000000,000000,000000,000000)}";
...
//	   須使用{\\K$PreOffset[$i]}{\\K$offset[$i]}。
	...
	$t91=$EventEnd-1000;
	$t92=$EventEnd-$t7;
	$s .= "{\\fad(300,300)}{\\K$PreOffset[$i]}{\\K$offset[$i]}{\\t($t91,$t92,1,\\alpha&H$scaleT}";
	$Upper .= "{\\fad(300,300)}{\\K$PreUpperOffset[$i]}{\\K$UpperOffset[$i]}{\\t($t91,$t92,1,\\alpha&H$scaleT}";
	if($color2 != 'FFFFFF'){
		//$t91=$end[$i]+$t7;
		//$t92=$EventEnd-$t7;
		$t7 = 500;
		...
		$t92=$EventEnd-$t7;
		$scaleT='88';
		$s .= "{\\t($t91,$t92,1,\\1vc(FFFFFF,FFFFFF,FFFFFF,FFFFFF)}";
		$Upper .= "{\\t($t91,$t92,1,\\1vc(FFFFFF,FFFFFF,FFFFFF,FFFFFF)}";

		$tmid = ($start[$i]+$end[$i])/2;
		$sClone .= "{\\fad(300,300)}{\\K$offset[$i]}{\\t($tmid,$end[$i],1,\\alpha&HFF}";
		$UpperClone .= "{\\fad(300,300)}{\\K$UpperOffset[$i]}{\\t($tmid,$end[$i],1,\\alpha&HFF}";
