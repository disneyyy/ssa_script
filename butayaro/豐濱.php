...
//	3. 以下兩行代碼是作定位用，若不想做其他位置特效，請保留這兩行代碼。
	...
	if($color[0] == $color[1] && $color[0] == $color[2] && $color[0] == $color[3]){
		if($color[0] == 'FFFFFF'){
			...
			$color[mt_rand(0,3)] = 'FFFFFF';
			$color[mt_rand(0,3)] = 'FFFFFF';
		}
	}
	if($c == 3){
		$a = mt_rand(0,3);
		while($color[$a] != 'FFFFFF'){
			$a = mt_rand(0,3);
		}
		$color[$a] = $color1;
	}
...
//	   須使用{\\K$PreOffset[$i]}{\\K$offset[$i]}。
	...
	for($j=0; $j<4; $j++){
		if($color[$j] != 'FFFFFF') $color[$j] = $color2;
	}
	$t7 = 500;
	$t91=$EventEnd-1000;
	$t92=$EventEnd-$t7;
	$scaleT='88';
	$s .= "{\\fad(300,300)}{\\K$PreOffset[$i]}{\\K$offset[$i]}{\\t($t91,$t92,1,\\1vc($color[0],$color[1],$color[2],$color[3])}";
	$Upper .= "{\\fad(300,300)}{\\K$PreUpperOffset[$i]}{\\K$UpperOffset[$i]}{\\t($t91,$t92,1,\\1vc($color[0],$color[1],$color[2],$color[3])}";
